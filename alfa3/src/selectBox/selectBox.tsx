...
export const SelectBox:React.FunctionComponent<SelectBoxProps> = ({ value, options, onChange }) => {
  const handleValueChange = (item: SelectBoxItem) => {
    console.log('handleValueChange', item)
    onChange(item)
  }
  return (
    ...
    <Listbox value={value} onChange={handleValueChange}>
      ...
      <Listbox.Options className="border-2 absolute p-2 mb-1 right-1 top-[2em] bg-white rounded-md">
        {options.map((option) => (
          <Listbox.Option
            className={`cursor-pointer py-1 px-2 hover:bg-slate-100 hover:rounded-full ${option.disabled ? 'text-gray-400 hover:cursor-not-allowed' : ''}`}
            key={option.id}
            value={option}
